...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = productSchema.parse({
      ...
      stockQuantity: form.get("stockQuantity") === "" ? null : form.get("stockQuantity"),
    });
    ...
    if (files.length) {
      ...
      for (const [position, file] of files.entries()) {
        if (!/^image\/(jpeg|png|webp)$/.test(file.type) || file.size > 5_000_000) {
          throw new Error("الصورة يجب أن تكون JPG أو PNG أو WebP وأصغر من 5MB.");
        }
        const extension = file.type.split("/")[1];
        const path = `${product.id}/${crypto.randomUUID()}.${extension}`;
        const { error } = await storage.storage.from(PRODUCTS_BUCKET).upload(path, Buffer.from(await file.arrayBuffer()), {
      ...
      await prisma.$transaction([
        prisma.productImage.createMany({ data: images }),
