...
export async function POST(request: Request) {
  try {
    ...
    const product = await prisma.product.findFirst({
      where: { id: input.productId, isActive: true, isAvailable: true },
    ...
    if (!whatsappNumber)
      return NextResponse.json(
        { error: "رقم WhatsApp لم يُضبط بعد." },
        { status: 503 },
      );
    ...
    const message = [
      ...
      `السعر: ${formatEgp(product.price)}`,
      `الكمية: ${input.quantity}`,
      `رقم الطلب: ${order.id}`,
      `اسم العميل: ${input.customerName}`,
      `رقم الموبايل: ${input.customerPhone}`,
